...
def method(f_x, f_y, f_z, w0: list, t: np.ndarray) -> np.ndarray:
    ...
    for i in range(1, t.shape[0]):
        k1_x = dt * f_x(x[i - 1], y[i - 1], z[i - 1])
        k1_y = dt * f_y(x[i - 1], y[i - 1], z[i - 1])
        k1_z = dt * f_z(x[i - 1], y[i - 1], z[i - 1])

        k2_x = dt * f_x(x[i - 1] + k1_x / 2, y[i - 1] + k1_y / 2, z[i - 1] + k1_z / 2)
        k2_y = dt * f_y(x[i - 1] + k1_x / 2, y[i - 1] + k1_y / 2, z[i - 1] + k1_z / 2)
        k2_z = dt * f_z(x[i - 1] + k1_x / 2, y[i - 1] + k1_y / 2, z[i - 1] + k1_z / 2)

        k3_x = dt * f_x(x[i - 1] + k2_x / 2, y[i - 1] + k2_y / 2, z[i - 1] + k2_z / 2)
        k3_y = dt * f_y(x[i - 1] + k2_x / 2, y[i - 1] + k2_y / 2, z[i - 1] + k2_z / 2)
        k3_z = dt * f_z(x[i - 1] + k2_x / 2, y[i - 1] + k2_y / 2, z[i - 1] + k2_z / 2)

        k4_x = dt * f_x(x[i - 1] + k3_x, y[i - 1] + k3_y, z[i - 1] + k3_z)
        k4_y = dt * f_y(x[i - 1] + k3_x, y[i - 1] + k3_y, z[i - 1] + k3_z)
        k4_z = dt * f_z(x[i - 1] + k3_x, y[i - 1] + k3_y, z[i - 1] + k3_z)
